...
Modified by Larry Fortna for MPCNC machine
...
Changelog 2016-08-07
Modified to restore cut feed rate after doing a G00 move.
"""
...
#            p = np[:]        
    ...
    def generate_raster_gcode(self, curve, laserPower, altfeed=None):
        ...
        #LGF made this an else condition and put feedrate back to normal after doing a G0 fast move
        if(cutFeed < self.options.Mfeed):
            gcode += 'G0 X'+str(curve['x'])+' Y'+str(curve['y'])+' F'+str(self.options.Mfeed)+'\n'
            gcode += 'G1 '+cutFeed
        else:
            gcode += 'G1 X'+str(curve['x'])+' Y'+str(curve['y'])+' '+cutFeed+'\n'
